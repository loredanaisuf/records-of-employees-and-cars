...
public class ServiceRemorca extends ServiceUtilizator {
    ...
    public List<Remorca> getTrails(String companyName){
        ...
        try {
            ...
            while(rs.next()){
                remorci.add(new Remorca(rs.getString("numar_inmatriculare_remorca"), rs.getString("firma"), rs.getString("numar_inmatriculare_masina"),rs.getInt("anul_fabricatiei"), rs.getString("itp"), rs.getString("asigurare")));
            }
    ...
    public Remorca getTrail(String idRemorca){
        try {
            ...
            ResultSet rs = ps.executeQuery();
            rs.next();
            return new Remorca(idRemorca,rs.getString("firma"), rs.getString("numar_inmatriculare_masina"), rs.getInt("anul_fabricatiei"), rs.getString("itp"), rs.getString("asigurare"));
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public void addTrail(Remorca remorca){
        try {
            PreparedStatement ps = connection.prepareStatement("INSERT INTO remorci(numar_inmatriculare_remorca, firma, numar_inmatriculare_masina, anul_fabricatiei, itp, asigurare) VALUES (?, ?, ? ,?, ?, ?)");
            ...
            ps.setString(6, remorca.getAsigurare());
            ps.executeUpdate();
    ...
    public void updateTrail(Remorca remorca){
        ...
        try {
            PreparedStatement ps = connection.prepareStatement("UPDATE remorci SET numar_inmatriculare_masina = ?, firma = ?, anul_fabricatiei = ?, itp = ?, asigurare = ? WHERE numar_inmatriculare_remorca = ?");
            ps.setString(6, remorca.getNrInmatriculareRemorca());
            ps.setString(1,remorca.getNrInmatriculareMasina());
            ps.setString(2,remorca.getFirma());
            ps.setInt(3, remorca.getAnulFabricatiei());
            ps.setString(4, remorca.getItp());
            ps.setString(5, remorca.getAsigurare());
            ps.executeUpdate();
